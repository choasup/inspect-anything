...
- **对我们的启示**：1-2B 学生模型在垂直场景完全够用，前提是训练数据质量（这正是飞轮提供的）；其训练配方可直接复用为我们 SFT pipeline 的骨架。**但仅复用配方、不用权重**：SmolVLM 无原生 grounding 训练目标（我们的核心原语它做不了），且小档位效果已被 Qwen3-VL 系反超。
...
- **对我们的启示**：纯 VLM 做密集小目标（货架上百个 SKU）的检测/计数仍偏弱且慢，学生模型采用**单视觉骨干 + 检测/LLM 双解码头**（详见 02 文档）；Grounded-SAM-2 流水线直接用于飞轮的自动预标注。

### 1.7 工业质检路线（检测范式统一）

- AnomalyGPT 等工作证明 VLM 可以做异常「有无 + 数量 + 位置」的归因描述；传统工业方案另有无监督异常检测一支（PatchCore 系，只用正常样本建模）。
- **我们的决策**：质检不引入独立的无监督异常检测模型，与零售/安防统一到**开放词汇检测范式**——缺陷即检测类别。缺陷样本稀缺改由「教师零样本伪标注 + 缺陷合成扩增 + 通用 defect 文本原型兜底」解决（详见 02/03 文档）。换来的是三个领域一套架构、一套训练管线、一份飞轮标注通吃。

### 1.8 视觉特征骨干系（DINOv3 / PE / SAM3）

学生模型共享骨干的三个候选来源，不在同一层，各占一格：

- **DINOv3**（Meta, 2025-08）：7B 自监督 ViT + Gram anchoring（解决高分辨率密集特征退化），冻结骨干即超过专用方案的密集任务 SOTA；官方蒸馏出 ConvNeXt-T/S/B/L 端侧友好变体；商用许可。短板：自监督出身**无原生文本对齐**，接 LLM 头或做开放词汇分类原型都要补对齐训练。
- **PE（Perception Encoder, Meta）**：核心论点是「一个对比学习骨干 + 不同对齐方式」（PE-lang 喂 LLM、PE-spatial 喂密集任务）——**与 inspect-one 单骨干双头在哲学上完全同构**。CLIP 式出身使文本对齐免费，连接器训练成本最低；dense 质量略逊 DINOv3。
- **SAM3**（Meta, 2025-11）：不是骨干，是完整的「可提示概念分割」系统——848M，共享视觉编码器 + DETR 检测器 + 视频跟踪器，文本概念 → 全部实例的框+掩码+跟踪；**presence head 把识别与定位解耦**，4M 概念词表含 hard negative 训练。图像/视频概念分割精度较此前系统翻倍。
- **对我们的启示**：① SAM3 验证了「共享编码器 + DETR 检测器」路线，直接做飞轮 L1 预标注（替代 Grounding DINO + SAM2 两级流水线）和学生检测头的蒸馏教师；② presence head 设计抄进检测头（误报压制）；③ 学生骨干权衡=「**对齐难补、dense 可练**」，故 PE 起步、DINOv3 做检测分支对照、Phase 3 多教师蒸馏合流。
...
| 8 | 密集实时检测不硬上自回归 VLM；检测头与 LLM 头共享视觉骨干 | YOLO-World / FastVLM | 学生模型 inspect-one 双头设计 |
| 9 | SAM3 单模型（框+掩码+跟踪）= 自动标注基础设施 + 检测蒸馏教师 | SAM3 | 飞轮预标注 / det 头蒸馏 |
| 10 | 偏好优化（MPO/DPO）对多模态推理有稳定增益 | InternVL3 | 训练第三阶段 |
| 11 | 质检统一检测范式：缺陷即类别，稀缺靠教师伪标注+合成扩增 | 1.7 节决策 | 质检产品线 |
| 12 | FP8/量化版本随模型一起发布是工程标配 | Qwen3-VL | 发布流程 |
| 13 | presence head：识别与定位解耦，直接改善固定召回下的误报率 | SAM3 | 学生检测头结构 |
| 14 | 学生骨干选型：对齐难补、dense 可练 → 文本对齐骨干（PE）起步 | PE vs DINOv3 | 学生骨干 Phase 1 |
